 ...
 ##This program estimate the parameters in a FOU(2) process, that is FOU(l,l) (with the same lambda).
...
n=length(x)
  ...
  U_Tn=function(l)
  {
    ...
    esp_dens=og^2*gamma(2*Hg+1)*sin(Hg*pi)*abs(j)^(3-2*Hg)/(2*pi*(l^2+j^2)^2)    
  ...
  lg=optim(c(0.1),U_Tn, lower=0.01,upper=1.5,method ="L-BFGS-B")$par
