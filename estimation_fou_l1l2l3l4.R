 ...
 ##This program estimate the parameters in a FOU(4) process, that is FOU(l1,l2,l3,l4) (with pairwise different lambda's).
...
#  a=c(-1,8,-28,56,-70,56,-28,8,-1)/256
  ...
  H_o_l1l2l3l4=function(x,Te,aa,bb)
  {
  x=x-mean(x)
  n=length(x)
...
n=length(x)
 ...
 U_Tn=function(beta)
  ...
  l4=beta[4]
    ...
    esp_dens=og^2*gamma(2*Hg+1)*sin(Hg*pi)*abs(j)^(7-2*Hg)/(2*pi*(l1^2+j^2)*(l2^2+j^2)*(l3^2+j^2)*(l4^2+j^2))    
    ...
    lg=constrOptim(c(0.3,0.5,0.8,0.9),U_Tn,NULL, ui=rbind(c(1,0,0,0),c(-1,1,0,0),c(0,-1,1,0),c(0,0,-1,1))
    ,ci=c(0.01,0.01,0.01,0.01))$par
  ...
  print("Estimation of parameters: H, sigma, lambda_1, lambda_2,lambda_3 and lambda_4")
  c(Hg,og,l1g,l2g,l3g,l4g) 
  }
