 ...
 ##This program estimate the parameters in a FOU(2) process, that is FOU(l1,l2) (with the different lambda's).
  ...
  ##(aa must be greater than or equal to 2p=4.
...
n=length(x)
  ...
  l2=beta[2]
    ...
    esp_dens=og^2*gamma(2*Hg+1)*sin(Hg*pi)*abs(j)^(3-2*Hg)/(2*pi*(l1^2+j^2)*(l2^2+j^2))    
    ...
    lg=constrOptim(c(0.2,0.6),U_Tn,NULL, ui=rbind(c(1,0),c(-1,1)),ci=c(0.01,0.01))$par

  l1g=min(lg)
  l2g=max(lg)
  
  print("Estimation of parameters: H, sigma, lambda_1 and lambda_2")
  c(Hg,og,l1g,l2g) 
  }
